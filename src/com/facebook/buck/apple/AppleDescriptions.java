...
public class AppleDescriptions {
  ...
  static AppleBundle createAppleBundle(
    ...
    // Albeit above comments, we (Airbnb) don't have any generated framework.
    // We are simply copying all transitive prebuilt framework dependencies to the bundle.
    ImmutableSet<TargetNode<?>> prebuiltFrameworkNodes = AppleBuildRules.getRecursiveTargetNodeDependenciesOfTypes(
      xcodeDescriptions,
      targetGraph,
      Optional.empty(),
      RecursiveDependenciesMode.COPYING,
      targetGraph.get(binaryTarget),
      ImmutableSet.of(PrebuiltAppleFrameworkDescription.class)
    );

    for (TargetNode<?> node : prebuiltFrameworkNodes) {
      if (!((PrebuiltAppleFrameworkDescriptionArg)node.getConstructorArg())
        .getPreferredLinkage().equals(NativeLinkableGroup.Linkage.STATIC)) {
        PrebuiltAppleFramework buildRule = (PrebuiltAppleFramework)graphBuilder.requireRule(node.getBuildTarget());
        frameworksBuilder.add(buildRule.getPlatformFrameworkPath(appleCxxPlatform.getCxxPlatform()));
      }
    }
