...
public class SwiftDescriptions {
  ...
  public static boolean hasSwiftSource(SourcePathResolverAdapter sourcePathResolverAdapter, ImmutableSet<SourceWithFlags> srcs) {
    return !filterSwiftSources(sourcePathResolverAdapter, srcs).isEmpty();
  }

  public static boolean isSwiftOnly(
    SourcePathResolverAdapter sourcePathResolverAdapter, ImmutableSet<SourceWithFlags> srcs) {
    if (srcs.isEmpty()) {
      // When the srcs is empty, it's likely the module is only for exposing headers,
      // Thus it's not a swift only module.
      return false;
    }
