...
public class SwiftCompile extends AbstractBuildRule implements SupportsInputBasedRuleKey {
  ...
  private SwiftCompileStep makeCompileStep(SourcePathResolverAdapter resolver) {
    ...
    compilerCommand.addAll(
      MoreIterables.zipAndConcat(Iterables.cycle("-Xcc"),
        MoreIterables.zipAndConcat(Iterables.cycle(INCLUDE_FLAG),
          getBuildDeps().stream()
            .filter(HeaderSymlinkTree.class::isInstance)
            .map(HeaderSymlinkTree.class::cast)
            .map(symlink -> resolver.getRelativePath(symlink.getRootSourcePath()).toString())
    ...
    if (vfsoverlayFilePath.isPresent()) {
      ...
      compilerCommand.add("-vfsoverlay", vfsoverlayFilePath.get().toString(), INCLUDE_FLAG, "/virtual");
    } else {
      compilerCommand.addAll(
  ...
  private SwiftCompileStep makeModulewrapStep(SourcePathResolverAdapter resolver) {
    ...
    ProjectFilesystem projectFilesystem = getProjectFilesystem();
    return new SwiftCompileStep(
        projectFilesystem.getRootPath(), argsFilePath, ImmutableMap.of(), compilerCommand.build(), Optional.empty(), Optional.empty());
  }
